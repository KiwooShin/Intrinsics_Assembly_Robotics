...
def main() -> None:
    """Benchmark compute variants and an in-RAM cached DataLoader."""
    import numpy as np  # noqa: F401 - kept for parity with the cached-loader path
    import torch
    ...
    def bench(bs, amp=False, clast=False, compile_=False, iters=150):
        ...
        for _ in range(10):
            step()              # warmup (compile builds here)
    ...
    for name, kw in [("bs64 baseline", dict(bs=64)),
        ...
        except Exception as e:  # noqa: BLE001 - benchmark harness reports and continues
    ...
    class Cached(Dataset):
        ...
        def __getitem__(self, i):
            return self.imgs[i].float(), self.st[i], self.act[i]
